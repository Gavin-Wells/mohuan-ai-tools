...
interface ClaudeSetupPageProps {
...
export function ClaudeSetupPage({ providers: _providers, onProvidersChanged: _onProvidersChanged }: ClaudeSetupPageProps) {
  const [apiKey, setApiKey] = useState(readSavedKey);
  const [status, setStatus] = useState<Status>(() => readSavedKey() ? "done" : "idle");
  const [statusMsg, setStatusMsg] = useState(() => readSavedKey() ? "已配置，可直接启动 Claude Code" : "");
  ...
  const handleSetup = useCallback(async () => {
    const k = apiKey.trim();
    if (!k) { toast.error("请先输入 API Key"); return; }
    setStatus("writing"); setStatusMsg("正在配置并启动 Claude Code...");
    try {
      const settingsConfig = buildMohuanClaudeSettings(k);
      const msg: string = await invoke("setup_and_launch_claude", { apiKey: k, settingsConfig });
  ...
  const handleRestore = useCallback(async () => {
    ...
    try {
      await invoke("restore_claude_defaults");
      localStorage.removeItem("claude_api_key");
      setApiKey(""); setBilling(null);
      setStatus("idle"); setStatusMsg("已还原 Claude Code 网关配置");
      toast.success("Claude Code 设置已还原");
    } catch (err) {
      setStatus("invalid");
  ...
  const claudeReady = installStatus === "installed";
    ...
    <div className="px-6 flex flex-col flex-1 min-h-0">
      <div className="max-w-md mx-auto w-full flex flex-col justify-center flex-1 gap-4 py-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <Button
            onClick={handleSetup}
            disabled={isLoading || !apiKey.trim() || !claudeReady}
            title={!claudeReady ? "请先安装 Claude Code CLI" : ""}
            className="h-10 text-xs font-semibold bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white shadow-sm"
          >
